...
const BookList = (props) => {
    ...
    const previousBooks = useRef()
    useEffect(() => previousBooks.current = filteredBooks)
    const prevBooks = previousBooks.current

    const searchInput = useRef(null)

    useEffect(() => {
        (prevBooks[0].asin !== props.books[0].asin) && filterBooks(searchInput.value)
    })
    ...
    return (<Container>
        <Row className="justify-content-center">
            <Col xs={12} className='d-flex justify-content-center my-4'>
                <span className="mr-3">Search:</span><input className='w-100' type="text" ref={e => searchInput.current = e} onChange={(e) => filterBooks(e.target.value)} />
